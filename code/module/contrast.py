...
#     def __init__(self, hidden_dim, tau, lam, num_negatives=5):
...
#         self.num_negatives = num_negatives  # 负样本数量
#
#         # 初始化投影层的权重
#         for model in self.proj:
#             if isinstance(model, nn.Linear):
...
#         Args:
#             z_mp (Tensor): Embeddings from view 'b' (e_m^{(b)}).
#             z_sc (Tensor): Embeddings from view 'a' (e_d^{(a)}).
#             positive_pairs (List[Tuple[int, int]]): List of (m, d) indices for positive pairs.
#             negative_pairs (List[List[int]]): List where each element is a list of negative 'q' indices for the corresponding (m, d).
#         Returns:
#             Tensor: SCL loss value.
#         """
#         m_indices = [pair[0] for pair in positive_pairs]
#         d_indices = [pair[1] for pair in positive_pairs]
#
#         # Get embeddings for positive pairs
#         e_m = z_mp[m_indices]  # Shape: (N, D)
#         e_d = z_sc[d_indices]  # Shape: (N, D)
#
#         # Compute similarity for positive pairs
#         sim_pos = F.cosine_similarity(e_m, e_d)  # Shape: (N,)
#
#         # Initialize tensors for negative similarities
#         sim_neg = torch.zeros_like(sim_pos)
#
#         # Iterate over each positive pair to compute negative similarities
#         for i, negatives in enumerate(negative_pairs):
#             if len(negatives) == 0:
#                 # If no negatives are sampled, use a very small value to avoid NaNs
#                 sim_neg[i] = -1e9
#             else:
#                 e_q = z_sc[negatives]  # Shape: (K, D)
#                 sim_q = F.cosine_similarity(e_m[i].unsqueeze(0), e_q)  # Shape: (K,)
#                 # Take the maximum similarity among negatives to make the loss harder
#                 sim_neg[i] = torch.logsumexp(sim_q / self.tau, dim=0)
#
#         # Compute the SCL loss
...
#         # Sample multiple negatives per positive for SCL
#         # Here, num_negatives is predefined (e.g., 5)
#         sampled_zero_index = []
#         for i in range(num_samples):
#             # Sample 'num_negatives' negatives for each positive
#             negatives = [zero_index[j][1] for j in
#                          range(i * self.num_negatives, (i + 1) * self.num_negatives)
#                          if (i * self.num_negatives + j) < len(zero_index)]
#             m = one_index[i][0]
#             sampled_zero_index.append((m, negatives))
#
#         # Extract positive pairs
#         positive_pairs = one_index[:num_samples]
#         z_mp = z_mp1+z_sc1
#         z_sc = z_mp2+ z_sc2
#         # Extract negative pairs corresponding to each positive
#         negative_pairs = [neg for (_, neg) in sampled_zero_index]
#         # Compute SCL loss
#         loss_scl = self.scl_loss(z_mp, z_sc, positive_pairs, negative_pairs)
#         # ------------------- SCL部分代码结束 -------------------
#
#         # 组合总损失
